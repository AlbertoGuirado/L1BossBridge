...
contract L1BossBridge is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public DEPOSIT_LIMIT = 100_000 ether; // e Limit to deposit in the vault

    IERC20 public immutable token; 
    L1Vault public immutable vault;
    mapping(address account => bool isSigner) public signers; // users who can send tokens from L1 -> L2
...
// @ audit-info no NATSPEC
     ...
     */
     //@ audit HIGH - no check for the <from> parameter [msg.sender] - An attacker can fake the address to deposit from another one and withdraw the stealed tokens  
     // @ audit HIGH The vault can be the from
    ...
    function sendToL1(uint8 v, bytes32 r, bytes32 s, bytes memory message) public nonReentrant whenNotPaused {
        ...
        // @ audit HIGH - An atacker can fake the message to approve and drain
        // @ audit data with crazy gas costs
